...
@property (nonatomic, copy) NSString * roomLoadingImgPath;

-(void)showAtView:(UIView *)view hudType:(TKHUDLoadingType)hudType;

-(void)tk_hide;

-(void)tk_hideAfterDelay:(NSTimeInterval)afterDelay;
...
+(void)showAtView:(UIView *)view bgColor:(UIColor *)color message:(NSString *)message hudType:(TKHUDLoadingType)hudType;

#pragma mark - Hide HUD

+(void)tk_hideForView:(UIView *)view;
