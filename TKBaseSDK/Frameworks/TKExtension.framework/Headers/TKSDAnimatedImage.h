...
/**
 This is the protocol for TKSDAnimatedImage class only but not for TKSDAnimatedImageCoder. If you want to provide a custom animated image class with full advanced function, you can conform to this instead of the base protocol.
 */
@protocol TKSDAnimatedImage <TKSDAnimatedImageProvider>
 ...
 */
- (nullable instancetype)initWithData:(nonnull NSData *)data scale:(CGFloat)scale options:(nullable TKSDImageCoderOptions *)options;
...
/**
 ...
 @param animatedCoder An animated coder which conform `TKSDAnimatedImageCoder` protocol
 @param scale The scale factor to assume when interpreting the image data. Applying a scale factor of 1.0 results in an image whose size matches the pixel-based dimensions of the image. Applying a different scale factor changes the size of the image as reported by the `size` property.
 @return An initialized object
 */
- (nullable instancetype)initWithAnimatedCoder:(nonnull id<TKSDAnimatedImageCoder>)animatedCoder scale:(CGFloat)scale;
...
/**
 The image class which supports animating on `TKSDAnimatedImageView`. You can also use it on normal UIImageView/NSImageView.
 */
@interface TKSDAnimatedImage : UIImage <TKSDAnimatedImage>
 ...
 */
@property (nonatomic, assign, readonly) TKSDImageFormat animatedImageFormat;
...
// By default, animated image frames are returned by decoding just in time without keeping into memory. But you can choose to preload them into memory as well, See the decsription in `TKSDAnimatedImage` protocol.
