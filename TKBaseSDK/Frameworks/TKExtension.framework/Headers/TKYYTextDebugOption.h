...
@class TKYYTextDebugOption;

NS_ASSUME_NONNULL_BEGIN

/**
 The TKYYTextDebugTarget protocol defines the method a debug target should implement.
 A debug target can be add to the global container to receive the shared debug
 option changed notification.
 */
@protocol TKYYTextDebugTarget <NSObject>
 ...
 */
- (void)setDebugOption:(nullable TKYYTextDebugOption *)option;
@end



/**
 The debug option for TKYYText.
 */
@interface TKYYTextDebugOption : NSObject <NSCopying>
 ...
 */
+ (void)addDebugTarget:(id<TKYYTextDebugTarget>)target;
...
/**
 ...
 @param target A debug target.
 */
+ (void)removeDebugTarget:(id<TKYYTextDebugTarget>)target;
 ...
 */
+ (nullable TKYYTextDebugOption *)sharedDebugOption;
 ...
 */
+ (void)setSharedDebugOption:(nullable TKYYTextDebugOption *)option;
