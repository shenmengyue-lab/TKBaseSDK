...
typedef void(^TKSDImageLoaderProgressBlock)(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL);
typedef void(^TKSDImageLoaderCompletedBlock)(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished);

#pragma mark - Context Options

/**
 A `UIImage` instance from `TKSDWebImageManager` when you specify `TKSDWebImageRefreshCached` and image cache hit.
 This can be a hint for image loader to load the image from network and refresh the image from remote location if needed. If the image from remote location does not change, you should call the completion with `TKSDWebImageErrorCacheNotModified` error. (UIImage)
 @note If you don't implement `TKSDWebImageRefreshCached` support, you do not need to care abot this context option.
 */
FOUNDATION_EXPORT TKSDWebImageContextOption _Nonnull const TKSDWebImageContextLoaderCachedImage;

#pragma mark - Helper method

/**
 This is the built-in decoding process for image download from network or local file.
 @note If you want to implement your custom loader with `requestImageWithURL:options:context:progress:completed:` API, but also want to keep compatible with TKSDWebImage's behavior, you'd better use this to produce image.
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable TKSDImageLoaderDecodeImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, TKSDWebImageOptions options, TKSDWebImageContext * _Nullable context);

/**
 This is the built-in decoding process for image progressive download from network. It's used when `TKSDWebImageProgressiveLoad` option is set. (It's not required when your loader does not support progressive image loading)
 @note If you want to implement your custom loader with `requestImageWithURL:options:context:progress:completed:` API, but also want to keep compatible with TKSDWebImage's behavior, you'd better use this to produce image.
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable TKSDImageLoaderDecodeProgressiveImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, BOOL finished,  id<TKSDWebImageOperation> _Nonnull operation, TKSDWebImageOptions options, TKSDWebImageContext * _Nullable context);

#pragma mark - TKSDImageLoader
...
/**
 ...
 If you want to implement custom loader for image download from network or local file, you just need to concentrate on image data download only. After the download finish, call `TKSDImageLoaderDecodeImageData` or `TKSDImageLoaderDecodeProgressiveImageData` to use the built-in decoding process and produce image (Remember to call in the global queue). And finally callback the completion block.
 ...
 @note It's your responsibility to load the image in the desired global queue(to avoid block main queue). We do not dispatch these method call in a global queue but just from the call queue (For `TKSDWebImageManager`, it typically call from the main queue).
*/
@protocol TKSDImageLoader <NSObject>
 ...
 */
- (nullable id<TKSDWebImageOperation>)requestImageWithURL:(nullable NSURL *)url
                                              ...
                                              completed:(nullable TKSDImageLoaderCompletedBlock)completedBlock;


/**
 Whether the error from image loader should be marked indded un-recoverable or not.
 If this return YES, failed URL which does not using `TKSDWebImageRetryFailed` will be blocked into black list. Else not.
