...
@protocol TKYYTextLinePositionModifier;
 ...
 */
extern const CGSize TKYYTextContainerMaxSize;

/**
 The TKYYTextContainer class defines a region in which text is laid out.
 TKYYTextLayout class uses one or more TKYYTextContainer objects to generate layouts.
 
 A TKYYTextContainer defines rectangular regions (`size` and `insets`) or 
 ...
 Example:
     ...
     └─────────────────────────────┘
 */
@interface TKYYTextContainer : NSObject <NSCoding, NSCopying>
...
/// The constrained size. (if the size is larger than TKYYTextContainerMaxSize, it will be clipped)
...
@property TKYYTextTruncationType truncationType;
...
@property (nullable, copy) id<TKYYTextLinePositionModifier> linePositionModifier;
@end


/**
 The TKYYTextLinePositionModifier protocol declares the required method to modify
 the line position in text layout progress. See `TKYYTextLinePositionSimpleModifier` for example.
 */
@protocol TKYYTextLinePositionModifier <NSObject, NSCopying>
@required
/**
 This method will called before layout is completed. The method should be thread-safe.
 @param lines     An array of TKYYTextLine.
 @param text      The full text.
 @param container The layout container.
 */
- (void)modifyLines:(NSArray<TKYYTextLine *> *)lines fromText:(NSAttributedString *)text inContainer:(TKYYTextContainer *)container;
@end


/**
 A simple implementation of `TKYYTextLinePositionModifier`. It can fix each line's position
 to a specified value, lets each line of height be the same.
 */
@interface TKYYTextLinePositionSimpleModifier : NSObject <TKYYTextLinePositionModifier>
...
/**
 TKYYTextLayout class is a readonly class stores text layout result.
 ...
 example: (layout with a circle exclusion path)
     ...
     └──────────────────────────┘
 */
@interface TKYYTextLayout : NSObject <NSCoding>
...
*/
+ (nullable TKYYTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text;
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable TKYYTextLayout *)layoutWithContainer:(TKYYTextContainer *)container text:(NSAttributedString *)text;
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable TKYYTextLayout *)layoutWithContainer:(TKYYTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range;

/**
 Generate layouts with the given containers and text.
 
 @param containers An array of TKYYTextContainer object (if nil, returns nil).
 @param text       The text (if nil, returns nil).
 @return An array of TKYYTextLayout object (the count is same as containers),
    or nil when an error occurs.
 */
+ (nullable NSArray<TKYYTextLayout *> *)layoutWithContainers:(NSArray<TKYYTextContainer *> *)containers
...
///< The text container
@property (nonatomic, strong, readonly) TKYYTextContainer *container;
///< The full text
...
///< Array of `TKYYTextLine`, no truncated
@property (nonatomic, strong, readonly) NSArray<TKYYTextLine *> *lines;
///< TKYYTextLine with truncated token, or nil
@property (nullable, nonatomic, strong, readonly) TKYYTextLine *truncatedLine;
///< Array of `TKYYTextAttachment`
@property (nullable, nonatomic, strong, readonly) NSArray<TKYYTextAttachment *> *attachments;
 ...
 */
- (nullable TKYYTextPosition *)closestPositionToPoint:(CGPoint)point;
...
/**
 ...
 @return A text position, or nil if not found.
 */
- (nullable TKYYTextPosition *)positionForPoint:(CGPoint)point
                                  oldPosition:(TKYYTextPosition *)oldPosition
                                otherPosition:(TKYYTextPosition *)otherPosition;
 ...
 */
- (nullable TKYYTextRange *)textRangeAtPoint:(CGPoint)point;
...
/**
 ...
 at point. Or nil if not found.
 */
- (nullable TKYYTextRange *)closestTextRangeAtPoint:(CGPoint)point;
 ...
 */
- (nullable TKYYTextRange *)textRangeByExtendingPosition:(TKYYTextPosition *)position;
...
/**
 ...
 @return The line index, or NSNotFound if not found.
 */
- (NSUInteger)lineIndexForPosition:(TKYYTextPosition *)position;
 ...
 */
- (CGPoint)linePositionForPosition:(TKYYTextPosition *)position;
 ...
 */
- (CGRect)caretRectForPosition:(TKYYTextPosition *)position;
...
/**
 ...
 If not found, it returns CGRectNull.
 */
- (CGRect)firstRectForRange:(TKYYTextRange *)range;
...
/**
 ...
 If not found, it returns CGRectNull.
 */
- (CGRect)rectForRange:(TKYYTextRange *)range;
...
/**
 ...
 @return An array of `TKYYTextSelectionRect` objects that encompass the selection.
 If not found, the array is empty.
 */
- (NSArray<TKYYTextSelectionRect *> *)selectionRectsForRange:(TKYYTextRange *)range;
...
/**
 ...
 If not found, the array is empty.
 */
- (NSArray<TKYYTextSelectionRect *> *)selectionRectsWithoutStartAndEndForRange:(TKYYTextRange *)range;
...
/**
 ...
 @param range An object representing a range in text.
 @return An array of `TKYYTextSelectionRect` objects contains the start and end to
 the selection. If not found, the array is empty.
 */
- (NSArray<TKYYTextSelectionRect *> *)selectionRectsWithOnlyStartAndEndForRange:(TKYYTextRange *)range;
...
- (void)drawInContext:(nullable CGContextRef)context
                ...
                debug:(nullable TKYYTextDebugOption *)debug
