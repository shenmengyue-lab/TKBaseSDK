...
typedef NS_ENUM(NSInteger, TKSDWebImageDownloaderExecutionOrder) {
    /**
     * Default value. All download operations will execute in queue style (first-in-first-out).
     */
    TKSDWebImageDownloaderFIFOExecutionOrder,
    
    /**
     * All download operations will execute in stack style (last-in-first-out).
     */
    TKSDWebImageDownloaderLIFOExecutionOrder
};
 ...
 */
@interface TKSDWebImageDownloaderConfig : NSObject <NSCopying>

/**
 Gets the default downloader config used for shared instance or initialization when it does not provide any downloader config. Such as `TKSDWebImageDownloader.sharedDownloader`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) TKSDWebImageDownloaderConfig *defaultDownloaderConfig;
...
/**
 * Gets/Sets a subclass of `TKSDWebImageDownloaderOperation` as the default
 * `NSOperation` to be used each time TKSDWebImage constructs a request
 * operation to download an image.
 * Defaults to nil.
 * @note Passing `NSOperation<TKSDWebImageDownloaderOperation>` to set as default. Passing `nil` will revert to `TKSDWebImageDownloaderOperation`.
 */
@property (nonatomic, assign, nullable) Class operationClass;

/**
 * Changes download operations execution order.
 * Defaults to `TKSDWebImageDownloaderFIFOExecutionOrder`.
 */
@property (nonatomic, assign) TKSDWebImageDownloaderExecutionOrder executionOrder;
