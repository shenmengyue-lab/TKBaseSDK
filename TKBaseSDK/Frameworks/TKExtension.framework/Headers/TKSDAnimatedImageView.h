...
/**
 ...
 Call `setImage:` with `UIImage(NSImage)` which conform to `TKSDAnimatedImage` protocol will start animated image rendering. Call with normal UIImage(NSImage) will back to normal UIImageView(NSImageView) rendering
 ...
 */
@interface TKSDAnimatedImageView : UIImageView
...
/**
 ...
 @note The best practice to use incremental load is using `initWithAnimatedCoder:scale:` in `TKSDAnimatedImage` with animated coder which conform to `SDProgressiveImageCoder` as well. Then call incremental update and incremental decode method to produce the image.
