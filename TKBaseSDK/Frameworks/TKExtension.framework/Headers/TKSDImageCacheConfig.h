...
typedef NS_ENUM(NSUInteger, TKSDImageCacheConfigExpireType) {
    /**
     * When the image is accessed it will update this value
     */
    TKSDImageCacheConfigExpireTypeAccessDate,
    /**
     * The image was obtained from the disk cache (Default)
     */
    TKSDImageCacheConfigExpireTypeModificationDate
};
 ...
 */
@interface TKSDImageCacheConfig : NSObject <NSCopying>

/**
 Gets the default cache config used for shared instance or initialization when it does not provide any cache config. Such as `TKSDImageCache.sharedImageCache`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) TKSDImageCacheConfig *defaultCacheConfig;
...
/*
 * The option to control weak memory cache for images. When enable, `TKSDImageCache`'s memory cache will use a weak maptable to store the image at the same time when it stored to memory, and get removed at the same time.
 ...
 */
@property (assign, nonatomic) TKSDImageCacheConfigExpireType diskCacheExpireType;
...
/**
 * The custom memory cache class. Provided class instance must conform to `TKSDMemoryCache` protocol to allow usage.
 * Defaults to built-in `TKSDMemoryCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initlized has no effect.
 */
@property (assign, nonatomic, nonnull) Class memoryCacheClass;

/**
 * The custom disk cache class. Provided class instance must conform to `TKSDDiskCache` protocol to allow usage.
 * Defaults to built-in `TKSDDiskCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initlized has no effect.
 */
