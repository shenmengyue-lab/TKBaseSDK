...
typedef NS_ENUM(NSUInteger, TKSDImageScaleMode) {
    TKSDImageScaleModeFill = 0,
    TKSDImageScaleModeAspectFit = 1,
    TKSDImageScaleModeAspectFill = 2
};

#if TKSD_UIKIT || TKSD_WATCH
typedef UIRectCorner TKSDRectCorner;
#else
typedef NS_OPTIONS(NSUInteger, TKSDRectCorner) {
    TKSDRectCornerTopLeft     = 1 << 0,
    TKSDRectCornerTopRight    = 1 << 1,
    TKSDRectCornerBottomLeft  = 1 << 2,
    TKSDRectCornerBottomRight = 1 << 3,
    TKSDRectCornerAllCorners  = ~0UL
};
#endif
 ...
 */
- (nullable UIImage *)sd_resizedImageWithSize:(CGSize)size scaleMode:(TKSDImageScaleMode)scaleMode;
...
- (nullable UIImage *)sd_roundedCornerImageWithRadius:(CGFloat)cornerRadius
                                              corners:(TKSDRectCorner)corners
